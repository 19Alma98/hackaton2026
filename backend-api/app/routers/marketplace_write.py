...
from app.schemas import (
    ...
    MarketplaceOfferDecisionRequest,
    MarketplaceOfferRequest,
    MarketplaceWithdrawOfferRequest,
    OfferInfo,
    TxResult,
)
...
@router.post("/offer", response_model=TxResult)
def make_offer(payload: MarketplaceOfferRequest):
    nft = _require_nft()
    marketplace = _require_marketplace()
    ...
    if owner.lower() == buyer.lower():
        raise HTTPException(status_code=400, detail="buyer_address cannot be token owner")

    existing_amount, existing_active = marketplace.functions.getOffer(payload.token_id, buyer).call()
    if existing_active:
        raise HTTPException(status_code=400, detail=f"offer already active with amount {existing_amount}")

    tx_fn = marketplace.functions.makeOffer(payload.token_id)
    return send_contract_transaction(
        from_address=buyer,
        contract_function=tx_fn,
...
@router.post("/offer/accept", response_model=TxResult)
def accept_offer(payload: MarketplaceOfferDecisionRequest):
    nft = _require_nft()
    marketplace = _require_marketplace()
    ...
    _, offer_active = marketplace.functions.getOffer(payload.token_id, buyer).call()
    if not offer_active:
        raise HTTPException(status_code=400, detail="no active offer for buyer")

    tx_fn = marketplace.functions.acceptOffer(payload.token_id, buyer)
    return send_contract_transaction(
        from_address=seller,
        contract_function=tx_fn,
        value_wei=0,
...
@router.post("/offer/reject", response_model=TxResult)
def reject_offer(payload: MarketplaceOfferDecisionRequest):
    nft = _require_nft()
    marketplace = _require_marketplace()
    ...
    tx_fn = marketplace.functions.rejectOffer(payload.token_id, buyer)
    return send_contract_transaction(
        from_address=seller,
        contract_function=tx_fn,
        value_wei=0,
...
@router.post("/offer/withdraw", response_model=TxResult)
def withdraw_offer(payload: MarketplaceWithdrawOfferRequest):
    marketplace = _require_marketplace()
    ...
    tx_fn = marketplace.functions.withdrawOffer(payload.token_id)
    return send_contract_transaction(
        from_address=buyer,
        contract_function=tx_fn,
        value_wei=0,
